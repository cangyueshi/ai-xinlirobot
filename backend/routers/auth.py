...
def _maybe_set_super_admin(user: User, openid: str, db: Session):
    if settings.SUPER_ADMIN_WECHAT_OPENID and openid == settings.SUPER_ADMIN_WECHAT_OPENID:
...
def wechat_login(data: WechatLogin, db: Session = Depends(get_db)):
    ...
    if user and user.role not in (UserRole.VISITOR, UserRole.SUPER_ADMIN):
        raise HTTPException(status_code=403, detail="咨询师和管理员不能从该入口登录，请使用管理后台")
    ...
    _maybe_set_super_admin(user, data.openid, db)
...
def mp_login(data: MpLoginRequest, db: Session = Depends(get_db)):
    ...
    _maybe_set_super_admin(user, openid, db)
...
@router.post("/counselor-register")
def counselor_register(
    ...
    db.add(counselor)
    db.commit()
    return {"ok": True, "message": "注册成功，请等待管理员审核", "is_approved": False}
